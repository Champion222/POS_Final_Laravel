<x-app-layout>
    ...
    <form method="POST" action="{{ route('profile.update') }}" enctype="multipart/form-data" class="max-w-5xl mx-auto">
        @csrf
        @method('PATCH')
            ...
            <div class="lg:col-span-1 space-y-6">
                ...
                <div class="bg-white rounded-[2rem] p-8 shadow-xl shadow-indigo-100/50 border border-gray-100 text-center relative overflow-hidden group">
                    ...
                    <div class="relative z-10 -mt-4">
                        ...
                        <p class="text-[11px] text-gray-400 mt-2">Your photo appears across POS, inventory, and reports.</p>

                        @error('image')
                            <p class="mt-3 text-xs font-bold text-red-500">{{ $message }}</p>
                        @enderror

                        <button id="save-photo-btn" type="submit" class="hidden mt-6 w-full py-2.5 rounded-xl bg-gray-900 text-white text-xs font-bold shadow-lg hover:bg-black transition-all animate-fade-in-up">
                            Save New Photo
                        </button>
                    </div>
                </div>
            ...
            <div class="lg:col-span-2">
                <div class="bg-white rounded-[2.5rem] shadow-xl shadow-gray-100/50 border border-gray-100 overflow-hidden">
                    ...
                    <div class="p-8 space-y-6">
                            ...
                            <div class="group">
                                <label class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2 ml-1">Email / Gmail Address</label>
                                <div class="relative">
                                    <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                            ...
                            @if(auth()->user()->role === 'admin')
                                <div class="pt-4 border-t border-gray-50">
                                    <h4 class="mb-4 text-sm font-bold text-gray-900">Security</h4>

                                    <div class="group">
                                        <label class="ml-1 mb-2 block text-xs font-bold uppercase tracking-wider text-gray-400">New Password <span class="ml-1 font-medium normal-case text-gray-300">(Leave blank to keep current)</span></label>
                                        <div class="relative">
                                            <div class="absolute inset-y-0 left-0 flex items-center pl-4 pointer-events-none">
                                                <i class="fas fa-lock text-gray-300 transition-colors group-focus-within:text-indigo-500"></i>
                                            </div>
                                            <input type="password" name="password" autocomplete="new-password"
                                                   class="w-full rounded-xl border-transparent bg-gray-50 py-3.5 pl-11 pr-4 font-bold text-gray-800 transition-all placeholder-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-4 focus:ring-indigo-500/10">
                                        </div>
                                    </div>

                                    <div class="group mt-4">
                                        <label class="ml-1 mb-2 block text-xs font-bold uppercase tracking-wider text-gray-400">Confirm Password</label>
                                        <div class="relative">
                                            <div class="absolute inset-y-0 left-0 flex items-center pl-4 pointer-events-none">
                                                <i class="fas fa-check-circle text-gray-300 transition-colors group-focus-within:text-indigo-500"></i>
                                            </div>
                            ...
                            @else
                                <div class="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-xs font-semibold text-slate-500">
                                    Password is managed by administrator. You can still update your profile photo, name, and email.
                                </div>
                            @endif
